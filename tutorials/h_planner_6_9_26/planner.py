...
class HierarchicalPlanner:
    ...
    def plan(self,
        ...
        if not p0_safe:
            ...
            return self._emergency_brake_trajectory(ego)
...
class PathExecutor:
    """
    Converts a Trajectory into VehicleControl commands.
    Uses a simple PID-style lateral controller and a proportional speed controller.
    ...
    KP_STEER              = 0.6    # proportional gain for steering
    KP_THROTTLE           = 0.5    # proportional gain for throttle
    MAX_STEER             = 0.7    # ±0.7 maximum steering input
    MAX_THROTTLE          = 0.8
    MAX_BRAKE             = 1.0
    ...
    def set_trajectory(self, traj: Trajectory):
        """Load a new trajectory and reset waypoint index."""
    ...
    def step(self, ego: carla.Actor) -> carla.VehicleControl:
        ...
        target_loc = traj.waypoints[wp_index]
        target_spd = traj.target_speeds[wp_index]

        ego_loc    = ego.get_location()
        ego_vel    = ego.get_velocity()
        ego_spd    = math.sqrt(ego_vel.x**2 + ego_vel.y**2)   # m/s
        ...
        steer       = max(-self.MAX_STEER, min(self.MAX_STEER,
                          self.KP_STEER * yaw_error))

        # ── Longitudinal control (throttle / brake) ──────────────────────────
        speed_error = target_spd - ego_spd

        if speed_error >= 0:
            throttle = min(self.MAX_THROTTLE, self.KP_THROTTLE * speed_error)
            brake    = 0.0
        else:
            throttle = 0.0
            brake    = min(self.MAX_BRAKE, -self.KP_THROTTLE * speed_error * 1.5)
