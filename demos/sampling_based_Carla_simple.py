...
def update_chase_camera(spectator: carla.Actor, vehicle: carla.Actor,
                       distance: float = 12.0, height: float = 5.0, pitch: float = -15.0):
    """
    Position camera behind and above vehicle (third-person chase view).
    
    This creates the classic driving game camera perspective.
    
    Args:
        spectator: CARLA spectator (camera)
        vehicle: Vehicle to follow
        distance: Distance behind vehicle (meters)
    ...
    camera_x = vehicle_loc.x - distance * math.cos(vehicle_yaw)
    camera_y = vehicle_loc.y - distance * math.sin(vehicle_yaw)
    camera_z = vehicle_loc.z + height
    
    camera_location = carla.Location(x=camera_x, y=camera_y, z=camera_z)
    
    # Look at vehicle from behind
    camera_rotation = carla.Rotation(
        pitch=pitch,  # Look slightly down
...
def visualize_all_trajectories(world: carla.World, trajectories: List[dict]):
    """
    Draw all candidate trajectories as waypoint markers in CARLA.
    ...
    for traj in trajectories:
        ...
        color = carla.Color(*traj['color'], 200)  # More opaque for points
        
        # Draw waypoints as small spheres
        # Skip some points to avoid clutter (every 3rd point)
        for i in range(0, len(waypoints), 3):
            wp = waypoints[i]
            
            debug.draw_point(
                carla.Location(x=wp['x'], y=wp['y'], z=2.0),
                size=0.08,  # Small sphere
                color=color,
                life_time=60.0
            )
    ...
    print("-" * 70)
    print(f"✓ Drew {len(trajectories)} trajectories as waypoint markers")


def visualize_selected_trajectory(world: carla.World, trajectory: dict):
    """
    Highlight the selected trajectory with larger waypoint markers.
    ...
    # Draw waypoints as larger yellow spheres (every 2nd point)
    for i in range(0, len(waypoints), 2):
        wp = waypoints[i]
        
        debug.draw_point(
            carla.Location(x=wp['x'], y=wp['y'], z=2.5),
            size=0.15,  # Larger for selected trajectory
...
def visualize_actual_path(world: carla.World, vehicle_positions: List[Tuple[float, float]]):
    """
    Draw the actual path driven by vehicle as waypoint markers.
    ...
    # Draw every 5th position to avoid too many points
    for i in range(0, len(vehicle_positions), 5):
        p = vehicle_positions[i]
        
        debug.draw_point(
            carla.Location(x=p[0], y=p[1], z=3.0),
            size=0.12,
...
def main():
    ...
    # How we score them based on the rulebook: 
        # TODO first: 
        # We need to implement scoring based on the rulebook to automatically select the best trajectory.
    ...
    # Position camera (third-person chase view)
    print("\n[STEP 6] Setting up chase camera...")
    spectator = world.get_spectator()
    update_chase_camera(spectator, vehicle, distance=12.0, height=5.0, pitch=-15.0)
    print("✓ Camera positioned behind and above vehicle")
    print("  Third-person chase view - like a racing game!")
    
    # Let user see all trajectories
    print("\n[STEP 7] Displaying all trajectories...")
    print("="*70)
    print("LOOK AT CARLA WINDOW!")
    print("  You should see ALL candidate trajectory WAYPOINTS:")
    print("    🔴 Red dots = Lane change LEFT trajectories")
    print("    🔵 Blue dots = Stay in lane trajectories")
    print("    🟢 Green dots = Lane change RIGHT trajectories")
    print("="*70)
    ...
    print("✓ Selected trajectory shown with YELLOW waypoint markers")
    ...
    print("  🟡 Yellow dots = Selected trajectory waypoints (planned)")
    print("  🟣 Magenta dots = Actual vehicle path (will appear as you drive)")
    print("  📹 Camera = Third-person chase view (behind vehicle)")
    print("="*70)
    ...
    try:
        while True:
            ...
            # Update camera to follow vehicle (chase view)
            update_chase_camera(spectator, vehicle, distance=12.0, height=5.0, pitch=-15.0)
    ...
    print("✓ Actual path drawn with MAGENTA waypoint markers")
    ...
    print(f"✓ Visualized all candidates as waypoint markers")
    ...
    print("  🔴 Red dots = Lane change LEFT options")
    print("  🔵 Blue dots = Stay in lane options")
    print("  🟢 Green dots = Lane change RIGHT options")
    print("  🟡 Yellow dots (larger) = Selected trajectory")
    print("  🟣 Magenta dots = Actual path driven")
    ...
    print("  ✓ Third-person chase camera (like racing games)")
